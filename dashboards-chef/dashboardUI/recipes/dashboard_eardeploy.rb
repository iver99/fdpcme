  ...
  code <<-EOH
    ...
    exit 0
        ...
        if  [ -z "$listApp" ]; then
            ...
            echo "#{node["jdk_dir"]}/jdk1.7.0_51/bin/java -cp #{node["wls_home"]}/server/lib/weblogic.jar weblogic.Deployer -username #{node["wls_admin_user"]} -password #{node["wls_admin_password"]} -url t3://#{node["hostname"]}:#{node["wls_port"]} -name #{node["myApplicationName"]} -deploy -targets #{node["target"]} -source #{node["apps_dir"]}/#{node["SAAS_servicename"]}/#{node["SAAS_version"]}/#{node["SAAS_earfile"]}"#{node["SAAS_version"]}.ea#{node["SAAS_version"]}.earr >> #{node["log_dir"]}/dashboardUIDatasource.log
