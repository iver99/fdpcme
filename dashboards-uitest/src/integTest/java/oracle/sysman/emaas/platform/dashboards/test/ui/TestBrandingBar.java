...
public class TestBrandingBar extends LoginAndLogout {
	...
	public void testAdminLinkITAAdmin() throws Exception {
		...
		Assert.assertEquals(url.substring(url.indexOf("emsaasui") + 9),
				"emcitas/warehouseadmin/html/admin-sources.html");
	...
	public void testAdminLinkLAAdmin() throws Exception {
		initTestCustom(
				Thread.currentThread().getStackTrace()[1].getMethodName(),
				"emaastesttenant1_la_admin1");
		webd.getLogger().info("start to test in testAdminLink");
		BrandingBarUtil.visitApplicationCloudService(webd,
				BrandingBarUtil.NAV_LINK_TEXT_CS_LA);
		WaitUtil.waitForPageFullyLoaded(webd);
		String url = webd.getWebDriver().getCurrentUrl();
		webd.getLogger().info("url = " + url);
		Assert.assertEquals(url.substring(url.indexOf("emsaasui") + 9),
	...
	public void testAdminLinkAPMAdmin() throws Exception {
		initTestCustom(
				Thread.currentThread().getStackTrace()[1].getMethodName(),
				"emaastesttenant1_apm_admin1");
		webd.getLogger().info("start to test in testAdminLink");
		// navigate to APM page
		BrandingBarUtil.visitApplicationCloudService(webd,
				BrandingBarUtil.NAV_LINK_TEXT_CS_APM);
		WaitUtil.waitForPageFullyLoaded(webd);
		String url = webd.getWebDriver().getCurrentUrl();
		webd.getLogger().info("url = " + url);
		Assert.assertEquals(url.substring(url.indexOf("emsaasui") + 9),
	...
	public void testAnalyzeLink() throws Exception {
		...
		// Analyze link
		BrandingBarUtil.visitApplicationVisualAnalyzer(webd,
				BrandingBarUtil.NAV_LINK_TEXT_VA_ITA);
		String url = webd.getWebDriver().getCurrentUrl();
		webd.getLogger().info("url = " + url);
		Assert.assertEquals(url.substring(url.indexOf("emsaasui") + 9),
	...
	/*
	 * @Test public void testAWRLink() throws Exception {
	 ...
	 * 
	 * //AWR Analytics link webd.click(DashBoardPageId.AWRALinkID);
	 * 
	 * String url = webd.getWebDriver().getCurrentUrl();
	 * webd.getLogger().info("url = "+url);
	 * Assert.assertEquals(url.substring(url.indexOf("emsaasui")+9),
	...
	public void testLogLink() throws Exception {
		...
		// Log link
		BrandingBarUtil.visitApplicationVisualAnalyzer(webd,
				BrandingBarUtil.NAV_LINK_TEXT_VA_LA);
		String url = webd.getWebDriver().getCurrentUrl();
		webd.getLogger().info("url = " + url);
		Assert.assertEquals(url.substring(url.indexOf("emsaasui") + 9),
				"emlacore/html/log-analytics-search.html");
	...
	public void testSearchLink() throws Exception {
		...
		// Target link
		BrandingBarUtil.visitApplicationVisualAnalyzer(webd,
				BrandingBarUtil.NAV_LINK_TEXT_VA_TA);
		String url = webd.getWebDriver().getCurrentUrl();
		webd.getLogger().info("url = " + url);
