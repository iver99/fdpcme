...
public class BugVerification extends LoginAndLogout
{
	...
	public void testEMCPDF_3120()
	{
		...
		Assert.assertFalse(webd.isDisplayed("id=" + DashBoardPageId.EXPLOREDATABTNID),
				"Explore Data menu is displayed in dashboard");
	}
	...
	public void testEMCPDF_3660() throws Exception
	{
		...
		webd.getLogger().info("Create a dashboard set");
		DashboardHomeUtil
				.createDashboardSet(webd, "DashboardSet_3660", "test for set custom time range for dashboard in the set");
		WaitUtil.waitForPageFullyLoaded(webd);
		...
		BrandingBarUtil.visitDashboardHome(webd);
		Assert.assertFalse(DashboardHomeUtil.isDashboardExisted(webd, "Dashboard_3660"),
				"Expected dashboard Dashboard_3660 is found in dashboard home page");
		...
		WaitUtil.waitForPageFullyLoaded(webd);
		Assert.assertNotNull(TimeSelectorUtil.setCustomTime(webd, OOB_idx, "04/07/2016 12:00 AM", "04/14/2016 12:30 PM"),
				"The return date time is null");
		...
		WaitUtil.waitForPageFullyLoaded(webd);
		Assert.assertNotNull(TimeSelectorUtil.setCustomTime(webd, newdsb_idx, "05/08/2016 12:00 AM", "05/15/2016 13:30 PM"),
				"The return date time is null");
	...
	public void testEMCPDF_4068_1()
	{
		...
		Assert.assertTrue(DashboardBuilderUtil.verifyDashboard(webd, "Dashboard_4068", null, true), "Create dashboard failed!");
		...
		Assert.assertFalse(webd.isDisplayed("css=" + DashBoardPageId.ERRORMSGSUMMARYCSS),
				"The error message isn't disappearred after re-input name in name input box");
		Assert.assertFalse(webd.isDisplayed("css=" + DashBoardPageId.ERRORMSGDETAILCSS),
				"The error message isn't disappearred after re-input name in name input box");
	...
	public void testEMCPDF_4068_2()
	{
		...
		Assert.assertTrue(DashboardBuilderUtil.verifyDashboardSet(webd, "DashboardSet_4068"), "Create dashboard failed!");
		...
		webd.sendKeys("css=" + DashBoardPageId.DASHBOARDSETOPTIONSEDITDESCRIPTIONCSS,
				"test the behaviour when clearing the input box of name field");
