...
import oracle.sysman.emaas.platform.dashboardscommon.ui.webutils.util.RegistryLookupUtil;
import oracle.sysman.emaas.platform.dashboardscommon.ui.webutils.util.StringUtil;
import oracle.sysman.emaas.platform.dashboardscommon.ui.webutils.wls.lifecycle.ApplicationServiceManager;
...
public class AvailabilityServiceManager implements ApplicationServiceManager, NotificationListener
{
	...
	{
		return "Dashboard Service Common UI Timer Service";
	}
	...
	public void handleNotification(Notification notification, Object handback)
	{
		...
		if (!rsm.isRegistrationComplete() && !rsm.registerService()) {
			logger.info("Dashboards Common UI service registration is not completed. Ignore dependant services availability checking");
			return;
		...
		if (!isSSFAvailable) {
			...
			logger.info("Dashboards Common UI service is out of service because Saved Search API service is unavailable");
			return;
		}
		...
		if (!isDFApiAvailable) {
			rsm.markOutOfService();
			logger.info("Dashboards Common UI service is out of service because Dashboard API service is unavailable");
			return;
		}
		...
		try {
			rsm.markServiceUp();
			logger.debug("Dashboards Common UI service is up");
		}
		catch (Exception e) {
			logger.error(e.getLocalizedMessage(), e);
		}
	...
	public void postStart(ApplicationLifecycleEvent evt) throws Exception
	{
		...
		notificationId = timer.addNotification("DashboardsCommonUIServiceTimer", null, this, timerTriggerAt, PERIOD, 0);
		timer.start();
		logger.info("Timer for dashboards common UI service dependencies checking started. notificationId={}", notificationId);
	}
	...
	public void preStop(ApplicationLifecycleEvent evt) throws Exception
	{
		...
		try {
			...
			logger.info("Timer for dashboards common UI dependencies checking stopped, notificationId={}", notificationId);
		}
