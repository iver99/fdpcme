    ...
    'ojs/ojbutton'
        ...
        function (oj, ko, $, _emJETCustomLogger, userTenantUtilModel, dfuModel, ctxModel/*, timeFilter*/) // this callback gets executed when all required modules are loaded
        {
            ...
            function MyViewModel() {
                var self = this;
                var start = new Date(new Date() - 30*24 * 60 * 60 * 1000);
                var end = new Date();
                ...
                self.start1 = ko.observable(self.dateConverter.format(tmpStart));
                self.end1 = ko.observable(self.dateConverter.format(tmpEnd));
                
                self.start2 = ko.observable(self.dateConverter.format(tmpStart));
                self.end2 = ko.observable(self.dateConverter.format(tmpEnd));
                ...
                self.timeParams1 = {
                    ...
                    callbackAfterApply: function (start, end, tp, tf, relTimeVal, relTimeUnit) {
                        ...
                        if((self.isTimePeriodLessThan1day(tp) || relTimeUnit==="SECOND" || relTimeUnit==="MINUTE" || relTimeUnit === "HOUR") && (start.getTimezoneOffset() !== end.getTimezoneOffset())) {
                            self.start1(self.dateConverter.format(appliedStart) + " ("+self.getGMTTimezone(start)+")");
                            self.end1(self.dateConverter.format(appliedEnd) + " ("+self.getGMTTimezone(end)+")");
                        }else {
                            self.start1(self.dateConverter.format(appliedStart));
                            self.end1(self.dateConverter.format(appliedEnd));
                        }
                    },
                ...
                self.timeParams2 = {
                    ...
                    callbackAfterApply: function (start, end, tp, tf, relTimeVal, relTimeUnit) {
                        ...
                        var appliedEnd = oj.IntlConverterUtils.dateToLocalIso(end);
                        if((self.isTimePeriodLessThan1day(tp) || relTimeUnit==="SECOND" || relTimeUnit==="MINUTE" || relTimeUnit === "HOUR") && (start.getTimezoneOffset() !== end.getTimezoneOffset())) {
                            self.start2(self.dateConverter.format(appliedStart) + " ("+self.getGMTTimezone(start)+")");
                            self.end2(self.dateConverter.format(appliedEnd) + " ("+self.getGMTTimezone(end)+")");
                        }else {
