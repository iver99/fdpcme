...
public class TestDateTimePicker extends CommonUIUtils
{
	public static void selectPeriod(WebDriver driver, String uicontrol, String period) throws Exception
	{
		...
		if (sTmpStartDay.equals(sTmpEndDay) && sTmpStartDay.equals("Today")) {
			...
			Assert.assertEquals(sDateTime, period + ": " + sTmpStartDay + " " + sTmpStartTime + " - " + sTmpEndDay + " "
					+ sTmpEndTime);
		}
	...
	private static String compareTime(Calendar current, String time)
	{
		...
		if (current.after(today)) {
			...
			return "Today";
				...
				&& current.get(Calendar.SECOND) == today.get(Calendar.SECOND)
				&& current.get(Calendar.MINUTE) == today.get(Calendar.MINUTE)
				&& current.get(Calendar.HOUR_OF_DAY) == today.get(Calendar.HOUR_OF_DAY)) { /*add fix for EMCPDF-1485. The fix is adding more "if" to resolve different time settings. In common API, there is a new implementation with some dev code fix.*/
			...
			return "Yesterday";
				...
				&& current.get(Calendar.DAY_OF_YEAR) == yesterday.get(Calendar.DAY_OF_YEAR)
				&& current.get(Calendar.SECOND) == yesterday.get(Calendar.SECOND)
				&& current.get(Calendar.MINUTE) == yesterday.get(Calendar.MINUTE)
				&& current.get(Calendar.HOUR_OF_DAY) == yesterday.get(Calendar.HOUR_OF_DAY)) {
	...
	private static String verifyDate(String time) throws ParseException
	{
		...
		return TestDateTimePicker.compareTime(current, time);
