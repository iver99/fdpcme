...
DECLARE
    V_COUNT NUMBER;
BEGIN
    --REM initialize newly introduced column 'ENABLE_REFRESH'
    SELECT COUNT(1) INTO V_COUNT FROM EMS_DASHBOARD WHERE TENANT_ID='&TENANT_ID' AND ENABLE_REFRESH=0;
    IF (V_COUNT=0) THEN
        DBMS_OUTPUT.PUT_LINE('ENABLE_REFRESH in EMS_DASHBOARD has been updated previously for tenant: &TENANT_ID, no need to update again');  
    ELSE
        UPDATE EMS_DASHBOARD SET ENABLE_REFRESH=ENABLE_TIME_RANGE WHERE TENANT_ID='&TENANT_ID';
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Update ENABLE_REFRESH in EMS_DASHBOARD successfully for tenant: &TENANT_ID');  
    END IF;
EXCEPTION
WHEN OTHERS THEN
    ROLLBACK;
    DBMS_OUTPUT.PUT_LINE('Failed to update ENABLE_REFRESH in EMS_DASHBOARD for tenant: &TENANT_ID due to '||SQLERRM);   
    RAISE;
END;
/
