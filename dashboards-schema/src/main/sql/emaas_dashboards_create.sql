...
Rem IS_SYSTEM - whether this is a system dashboard or not. A system dashbaoard can't be deleted. 0: not system dashboard, 1: is system dashboard
...
CREATE TABLE EMS_DASHBOARD
  (
    ...
    CONSTRAINT EMS_DASHBOARD_PK PRIMARY KEY (DASHBOARD_ID) USING INDEX,
    CONSTRAINT EMS_DASHBOARD_U1 UNIQUE (NAME, OWNER,TENANT_ID,DELETED)
  ) ;
  
prompt EMS_DASHBOARD_SEQ
CREATE SEQUENCE EMS_DASHBOARD_SEQ  MINVALUE 1 MAXVALUE 999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 ORDER  NOCYCLE;

prompt EMS_DASHBOARD_TR
CREATE OR REPLACE TRIGGER  EMS_DASHBOARD_TR BEFORE INSERT ON EMS_DASHBOARD
FOR EACH ROW
BEGIN
...
CREATE TABLE EMS_DASHBOARD_FAVORITE
  (
    ...
    CONSTRAINT EMS_DASHBOARD_FAVORITE_FK1 FOREIGN KEY (DASHBOARD_ID) REFERENCES EMS_DASHBOARD (DASHBOARD_ID)
  ) ;
...
CREATE TABLE EMS_DASHBOARD_TILE
  (
    ...
    WIDGET_ICON              VARCHAR2(1024) NOT NULL,
    WIDGET_HISTOGRAM         VARCHAR2(1024) NOT NULL,
    ...
    CONSTRAINT EMS_DASHBOARD_TILE_PK PRIMARY KEY (TILE_ID) USING INDEX,
    CONSTRAINT EMS_DASHBOARD_TILE_FK1 FOREIGN KEY (DASHBOARD_ID) REFERENCES EMS_DASHBOARD (DASHBOARD_ID)
  ) ;

prompt EMS_DASHBOARD_TILE_SEQ
CREATE SEQUENCE EMS_DASHBOARD_TILE_SEQ  MINVALUE 1 MAXVALUE 999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 ORDER  NOCYCLE;

prompt EMS_DASHBOARD_TILE_TR  
CREATE OR REPLACE TRIGGER  EMS_DASHBOARD_TILE_TR BEFORE INSERT ON EMS_DASHBOARD_TILE
FOR EACH ROW
BEGIN
...
CREATE TABLE EMS_DASHBOARD_TILE_PARAMS
  (
    ...
    CONSTRAINT EMS_DASHBOARD_TILE_PARAMS_PK PRIMARY KEY (TILE_ID, PARAM_NAME) USING INDEX,
    CONSTRAINT EMS_DASHBOARD_TILE_PARAMS_FK1 FOREIGN KEY (TILE_ID) REFERENCES EMS_DASHBOARD_TILE (TILE_ID)
  ) ;
